...
export default function Navbar({ onOpenCFP, onOpenTickets }: NavbarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-2.5 sm:px-4 py-2.5 sm:py-3.5 transition-all duration-300">
      <div
        className={`rounded-full transition-all duration-500 ease-in-out flex items-center justify-between px-3.5 sm:px-5 py-2 ${
          compact ? "w-full max-w-6xl" : "w-full max-w-7xl"
        } ${
          scrolled
      ...
      >
        {/* Brand Logo & Name */}
        <Link href="/" className={`flex items-center shrink-0 transition-all duration-300 ${compact ? "gap-1.5 sm:gap-2" : "gap-2 sm:gap-2.5"}`}>
